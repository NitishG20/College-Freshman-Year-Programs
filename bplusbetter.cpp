...
node *delet_entry( node * root, node * n, int key, record * pointer );
...
node * make_node( void ) {
    ...
    new_node->pointers =(node **) malloc( order * sizeof(void *) );
...
node * insert_into_leaf_after_splitting(node * root, node * leaf, int key, record * pointer) {
    ...
    temp_pointers =(node **) malloc( order * sizeof(node *) );
...
node * merge_nodes(node * root, node * n, node * neighbor, int neighbor_index, int k_prime) {
    ...
    if (!split) {
        root = delet_entry(root, n->parent, k_prime, n);
        free(n->keys);
        free(n->pointers);
        free(n);
    }
...
// delets an entry from the B+ tree.
 node * delet_entry( node * root, node * n, int key, node * pointer ) {
...
node * delet(node * root, int key) {
    ...
    if (key_record != NULL && key_leaf != NULL) {
        root = delet_entry(root, key_leaf, key, key_record);
        free(key_record);
    }
    return root;
}
